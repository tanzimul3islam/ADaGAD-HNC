...
PREPARERS = {
    "cora": lambda **kw: prepare_mat_dataset("cora", **kw) if Path("dataset/cora.mat").exists() else _legacy_planaroid("cora", **kw),
    "citeseer": lambda **kw: prepare_mat_dataset("citeseer", **kw),
    "pubmed": lambda **kw: prepare_mat_dataset("pubmed", **kw),
...
def _legacy_planaroid(name: str, **kwargs):
    from torch_geometric.datasets import Planetoid
    dataset_map = {"cora": "Cora"}
    dataset = Planetoid(root=kwargs.get("data_dir", "data"), name=dataset_map[name])
    data = dataset[0]
    _inject_anomalies(data, kwargs.get("anomaly_ratio", 0.05), kwargs.get("seed", 42))
    _save(data, Path(kwargs.get("data_dir", "data")) / name / "processed" / "data.pt")
    return _summary(data, name)
